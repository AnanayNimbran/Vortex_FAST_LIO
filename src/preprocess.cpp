...
void Preprocess::process(const sensor_msgs::msg::PointCloud2::UniquePtr &msg, PointCloudXYZI::Ptr& pcl_out)
{
  ...
  switch (lidar_type)
  {
    ...
    case VELO16:
      default_handler(msg); //velodyne_handler(msg); ye meine change kiya
      break;
...
/*ye niche wala block replace kiya hai..iske niche wale block se*/
...
//   }
// }
/*ye upar wala block iss niche wale block se replace kiya*/
...
//   // We calculate how many points are in the raw byte buffer
//   int plsize = msg->width * msg->height;
//   if (plsize == 0) return;
//   pl_surf.reserve(plsize);

//   float scan_duration = (1000.0f / static_cast<float>(SCAN_RATE));; //for varying rate .....and duration is in ms

//   for (int i = 0; i < plsize; i++)
//   {
//     // Basic C++: Get the starting memory address for point 'i'
//     const uint8_t* ptr = &msg->data[i * msg->point_step];

//     // Pointer casting: Tell C++ to read these specific bytes as floats/uint16
//     float x = *reinterpret_cast<const float*>(ptr + 0);  // Offset 0
//     float y = *reinterpret_cast<const float*>(ptr + 4);  // Offset 4
//     float z = *reinterpret_cast<const float*>(ptr + 8);  // Offset 8
//     uint16_t raw_i = *reinterpret_cast<const uint16_t*>(ptr + 12); // Offset 12 (uint16)

//     // The conversion you asked for:
//     float intensity = static_cast<float>(raw_i);
...
//     added_pt.normal_y = 0;
//     added_pt.normal_z = 0;
//     // THE FIX: Assign a linear time offset to each point (in milliseconds)
//     // This tells the EKF that the points were scanned one after another.
//     added_pt.curvature = (static_cast<float>(i) / plsize) * scan_duration;
//     //added_pt.curvature = 0;
...
//     // scan_duration in ms, derived from SCAN_RATE set by yaml
//     float scan_duration_ms = 1000.0f / static_cast<float>(SCAN_RATE);
...
//         uint32_t raw_i = *reinterpret_cast<const uint32_t*>(ptr + 12); // uint32, not uint16

//         if (!std::isfinite(x) || !std::isfinite(y) || !std::isfinite(z)) continue;

//         PointType added_pt;
//         added_pt.x=y;//added_pt.x = x;
//         added_pt.y=-x;//added_pt.y = y;
//         added_pt.z=z;//added_pt.z = z;
//         added_pt.intensity = static_cast<float>(raw_i);
//         added_pt.normal_x = 0;
//         added_pt.normal_y = 0;
//         added_pt.normal_z = 0;

//         // Linear time ramp: 0 → scan_duration_ms (in ms, as FAST-LIO expects)
//         added_pt.curvature = (static_cast<float>(i) / static_cast<float>(plsize)) * scan_duration_ms;
...
void Preprocess::default_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
    ...
    for (int i = 0; i < plsize; i++)
    {
        // Jump to the exact 20-byte memory block for point 'i'
        const uint8_t* ptr = &msg->data[i * msg->point_step];

        // Read the exact 4-byte chunks based on your terminal output
        float x = *reinterpret_cast<const float*>(ptr + 0);
        float y = *reinterpret_cast<const float*>(ptr + 4);
        float z = *reinterpret_cast<const float*>(ptr + 8);
        uint32_t raw_i = *reinterpret_cast<const uint32_t*>(ptr + 12);
        
        // THE MAGIC BYTES: Read the Little-Endian timestamp at offset 16
        uint32_t time_offset_ns = *reinterpret_cast<const uint32_t*>(ptr + 16);

        if (!std::isfinite(x) || !std::isfinite(y) || !std::isfinite(z)) continue;

        PointType added_pt;
        
        // Keeping your custom axis swaps
        added_pt.x = y;
        added_pt.y = -x;
        added_pt.z = z;
        added_pt.intensity = static_cast<float>(raw_i);
        ...
        // FAST-LIO's deskewer expects 'curvature' to be stored in milliseconds.
        // We divide your nanoseconds by 1,000,000 to convert to ms.
        added_pt.curvature = static_cast<float>(time_offset_ns) / 1000000.0f;
